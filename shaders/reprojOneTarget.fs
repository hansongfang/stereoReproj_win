...
vec4 reShading()
{
  ...
  vec3 vLightPos = vec3(curWorldView * vec4(lightPos, 1.0));//light pos from world space -> view 

  vec4 color = vec4(uAmbientLightColor * noise_color, 1.0);
...
vec4 reprojColor(vec4 targetPos, sampler2D tDiffuse)
{
  ...
  vec4 tempColor = texture2D(tDiffuse, targetCoord);
  vec3 repColor = tempColor.rgb;
  //float repDepth = tempColor.a;
  float repDepth = texture2D(tDepth, targetCoord).r;
...
void main()
{
  ...
  // float fragDepth = (targetPos_ndc.z + 1.0) / 2.0;
  // float repDepth = texture2D(tDepth, targetCoord).r;
  // if(abs(repDepth - fragDepth) < 0.0001){
  ...
  //color = vec4(0.0, 1.0, 0.0, 1.0);
  vec4 temp = reprojColor(targetPos, tDiffuse);
  vec3 repColor = temp.rgb;
  float repDiff = temp.a;
  if(repDiff < threshold && repDiff > epsilon){
    color = vec4(repColor, 1.0);
    // color = vec4(0.0, 0.0, 1.0, 1.0);
  }
  else if(repDiff <= epsilon){
    // color = reShading();
    color = vec4(0.0, 1.0, 0.0, 1.0);
  }
  else if(repDiff >= threshold){
    // color = vec4(1.0, 0.0, 0.0, 1.0);
    discard;
  }
}
